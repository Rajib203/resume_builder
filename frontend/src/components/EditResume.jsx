...
import { AdditionalInfoForm, CertificationInfoForm, EducationDetailsForm, ProfileInfoForm, ProjectDetailForm, SkillsInfoForm } from "./Forms";
import { ContactInfoForm } from "./Forms";
import { WorkExperienceForm } from "./Forms";
import RenderResume from './RenderResume'
import ThemeSector from "./ThemeSector";
...
const EditResume = () => {
  ...
  try {
    ...
    // If you have a profile image file input
    const profileFile = profileFileRef.current?.files[0]; // optional
  ...
  return (
    <DashboardLayout>
      ...
      <Modal isOpen={openPreviewModal} onClose={()=>setOpenPreviewModal(false)}
        title={resumeData.title}
        showActionBtn
        actionBtnText={isDownloading ? "Generating..":downloadSuccess ? "Downloades!":"Download PDF"}
        actionBtnIcon={
          isDownloading ? (
            <Loader2 size={16} className="animate-spin"/>
          ):downloadSuccess ? (
